drop view public.attendance_with_user;
drop view public.employee_analytics_summary_all_time_view;

ALTER TABLE public.attendance_records 
ADD COLUMN clock_in_new timestamp with time zone,
ADD COLUMN lunch_out_new timestamp with time zone,
ADD COLUMN lunch_in_new timestamp with time zone,
ADD COLUMN clock_out_new timestamp with time zone;

UPDATE public.attendance_records 
SET 
    clock_in_new = (work_date + clock_in) AT TIME ZONE 'UTC',
    lunch_out_new = CASE 
        WHEN lunch_out IS NOT NULL THEN (work_date + lunch_out) AT TIME ZONE 'UTC'
        ELSE NULL 
    END,
    lunch_in_new = CASE 
        WHEN lunch_in IS NOT NULL THEN (work_date + lunch_in) AT TIME ZONE 'UTC'
        ELSE NULL 
    END,
    clock_out_new = CASE 
        WHEN clock_out IS NOT NULL THEN (work_date + clock_out) AT TIME ZONE 'UTC'
        ELSE NULL 
    END;

ALTER TABLE public.attendance_records 
DROP COLUMN clock_in,
DROP COLUMN lunch_out,
DROP COLUMN lunch_in,
DROP COLUMN clock_out;

ALTER TABLE public.attendance_records 
RENAME COLUMN clock_in_new TO clock_in;
ALTER TABLE public.attendance_records 
RENAME COLUMN lunch_out_new TO lunch_out;
ALTER TABLE public.attendance_records 
RENAME COLUMN lunch_in_new TO lunch_in;
ALTER TABLE public.attendance_records 
RENAME COLUMN clock_out_new TO clock_out;
...
-- recreate view with new calculation
...
select
        ...
        STDDEV(
          (
            EXTRACT(
              hour
              from
                clock_in AT TIME ZONE 'Asia/Kuala_Lumpur'
            ) * 60
          ) + EXTRACT(
            minute
            from
  ...
  COALESCE(
    (
      select
        JSONB_AGG(
          JSONB_BUILD_OBJECT('name', name, 'date', date)
          order by
            date
        )
      from
