...
export default async function Page(props: { searchParams: SummarySearchParams }) {
        ...
        {isAdmin ? (
          <>
            ...
            <div className="flex h-full flex-col items-center justify-center gap-2 rounded-md border p-4 text-center">
              <p>
                {`${selectedUserId === userMetadata.employee_id ? "You'll see summaries here once you have" : "This employee's summaries will be available after"}  ${MIN_DAYS_FOR_SUMMARIES - summary.total_days} more days of attendance.`}
              </p>
            </div>
          </>
        ) : (
  ...
  return (
    <Summary summary={summary} dailyDataRecord={resolvedDailyDataRecord} users={resolvedUsers} user={userMetadata} />
  );
}
